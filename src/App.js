...
function App() {
  ...
  return (
    <div className={`flex flex-col items-center min-h-screen w-screen bg-dark p-8 ${connected ? null : 'justify-center'}`}>      
      {
        connected ? (
          ...
          <div className='w-full max-w-5xl flex flex-col text-white'>
            ...
            <div className='flex flex-col md:flex-row'>
              <div className='w-full border border-accent rounded-lg md:p-8 md:mr-2'>
                  <span className='text-secondary text-xs'><i className="cfu-block mr-2"></i>CONNECTION DETAILS</span>
                  ...
                  <div className='flex flex-row items-center mt-4 text-offWhite'>
                    ...
                    <span className='text-xs md:text-sm ml-2'>{network && networks.filter(n => n.id === network)[0].name}</span>
                  </div>
              ...
              <div className='w-full border border-accent rounded-lg md:p-8 my-4'>
                  <span className='text-secondary text-xs'><i className="cfu-nft mr-2"></i>NFTs</span>
                  <div className='flex flex-row flex-wrap items-center'>
                    { 
                      apes.length > 0 ? 
                      apes.map((ape, indx) => (
                        <img src={ape} alt="ape" key={indx} className="md:w-64 md:m-4 my-4" />
                        )
                      )
                      :
                      !searching ? (
                      <div className='flex flex-col w-full items-center justify-center'>
                        <span className='text-secondary text-xs my-4'>You have no NFTs ( BAYC's )</span>
                        <span>Search another address for BAYC's</span>
                        <input type="text" value={searchVal} placeholder={account} className='text-xs my-4 rounded-md h-12 w-1/3 max-w-sm outline-none text-black px-4'
                          onChange={(e) => {
        ...
        <button className='text-white' onClick={connectMetamask}>Connect wallet</button>
      }
